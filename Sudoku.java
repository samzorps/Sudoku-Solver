...
public class Sudoku {    
    ...
    /*
     *this 2-d array allows us to determine if a given row has a certain value
     *for example rowHasVal[4][9] would return true if row 4 already has a 9
     */
    private boolean[][] rowHasVal;
    
    /*
     *this 2-d array allows us to determine if a given column has a certain value
     *for example colHasVal[0][7] would return true if col 0 already has a 7
     */
    ...
    public boolean canPlace(int val, int row, int col) {
    	//returns true if can place a certain value in a given row, col
    	boolean ans = !(this.colHasVal[col][val] || this.rowHasVal[row][val] || this.subgridHasVal[row/3][col/3][val]); 
    	return ans;
    }
    ...
    private boolean solveRB(int n) {
        ...
        for(int v=1; v<10; v++) {
        	if ( this.canPlace(v, row, col) ){
        		this.placeVal(v, row, col);
        		
        		int skip = 1;
        		while( n+skip<81 && this.valIsFixed[(n+skip)/9][(n+skip)%9]) { // finds next non fixed digit value of n
        			skip++;
        		}
        		if (n+skip>80) {
        			return true;
        		...
        		RBRest = solveRB(n+skip);
        		
        		if ( !(RBRest) ){
            		this.removeVal(v, row, col);
            	} else {
            		return true;
            	}
